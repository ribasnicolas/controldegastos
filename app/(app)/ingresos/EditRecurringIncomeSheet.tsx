...
import { updateRecurringIncome } from "@/lib/actions/recurring";
...
export function EditRecurringIncomeSheet({
  item,
  categories,
  onClose,
}: {
  ...
  const [state, formAction] = useActionState(updateRecurringIncome, initialActionState);
        ...
        <form action={formAction} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                id={`edit-rec-income-amount-${item.id}`}
                name="amount"
                type="number"
                inputMode="decimal"
                step="0.01"
            ...
            <div>
              ...
              <input
                id={`edit-rec-income-day-${item.id}`}
                name="dayOfMonth"
                type="number"
                min="1"
                max="28"
          ...
          <div>
            ...
            <input
              id={`edit-rec-income-description-${item.id}`}
              name="description"
              type="text"
              maxLength={200}
